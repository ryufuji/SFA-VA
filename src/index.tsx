...
// 銀行入金一覧取得
app.get('/api/bank-deposits', authMiddleware, requirePermission('payment_manage'), async (c) => {
  const { DB } = c.env
  const { results } = await DB.prepare(`
    SELECT bd.*,
...
// 銀行入金詳細取得（消込履歴含む）
app.get('/api/bank-deposits/:id', authMiddleware, requirePermission('payment_manage'), async (c) => {
  const { DB } = c.env
  const id = c.req.param('id')
...
// 銀行入金登録
app.post('/api/bank-deposits', authMiddleware, requirePermission('payment_manage'), async (c) => {
  const { DB } = c.env
...
// 消込実行
app.post('/api/bank-deposits/:id/allocate', authMiddleware, requirePermission('payment_manage'), async (c) => {
  const { DB } = c.env
